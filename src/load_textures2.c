...
/*   Updated: 2024/03/24 13:47:56 by ajehle           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_load_textures_floor(t_game *game, char **map_as_arr)
{
	...
	while (map_as_arr[y])
	{
		...
		{
			if (mlx_image_to_window(game->game_window, floor_i, x * PIXEL, y
					* PIXEL))
			{
				// checking and exiting!!!!
			}
...
void	ft_load_textures_obstacle(t_game *game, char **map_as_arr)
{
	...
	while (map_as_arr[y])
	{
		while (map_as_arr[y][x])
		{
			if (map_as_arr[y][x] == '1')
			{
				if (mlx_image_to_window(game->game_window, obstacle_i, x
						* PIXEL, y * PIXEL))
				{
					// checking and exiting!!!!
				}
...
int	ft_load_textures_player(t_game *game)
{
	...
	if (mlx_image_to_window(game->game_window, game->player->image,
			game->player->pos->x * PIXEL, game->player->pos->y * PIXEL))
		return (call_exit(game), 0);
	return (0);
}
...
int	ft_load_textures_exit(t_game *game)
{
	...
	if (mlx_image_to_window(game->game_window, game->exit->image_closed,
			game->exit->pos->x * PIXEL, game->exit->pos->y * PIXEL))
		return (call_exit(game), 0);
